...
مشاوره و قیمت‌گذاری — هر خط زیر عیناً روی صفحه‌ی /consult سایت نوشته شده است. وقتی این ارقام را می‌گویی، حتماً اضافه کن که «در صفحهٔ /consult نوشته شده»، تا کاربر بداند عدد از خودت نیست و بتواند خودش ببیند:
- در صفحه‌ی /consult نوشته شده: «اولین گفت‌وگو هزینه‌ای ندارد.»
- در صفحه‌ی /consult نوشته شده: «رقم ثابتی ندارم، چون دامنهٔ کار ثابت نیست — بازطراحی یک فرایند با سیستم‌سازی کل یک مجموعه فرق دارد. بیشتر پروژه‌ها در بازهٔ ۵۰ تا ۱۵۰ میلیون تومان جمع می‌شوند؛ کارهای کوچک‌تر و محدودتر زیر آن، و مجموعه‌های بزرگ‌تر بالای آن.» عدد دقیق بعد از گفت‌وگوی اول مشخص می‌شود. این تنها بازه‌ای است که اعلام شده — بازه یا رقم دیگری نساز و این بازه را به تعرفه‌ی ساعتی یا قیمت پکیج تبدیل نکن.
- برای شروع باید از صفحه‌ی /consult فرم درخواست مشاوره پر شود.
- در صفحه‌ی /contact نوشته شده که بعد از ارسال فرم، ظرف ۴۸ ساعت کاری جواب داده می‌شود — حتی اگر جواب این باشد که کار در حوزه‌ی کاری او نیست.

راه‌های تماس (صفحه‌ی /contact):
- تلفن: ${site.phone.display} — فقط در ساعت کاری جواب داده می‌شود.
- ساعت کاری: ${site.hours.display}
- نشانی: ${site.address.display}
- ایمیل: ${site.email}
${socialLines}
- واتس‌اپ و ایمیل روزانه دیده می‌شوند؛ اینستاگرام و لینکدین کندتر.
